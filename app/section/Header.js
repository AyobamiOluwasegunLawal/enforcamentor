...
useEffect(() => {
  ...
  return (
    <section>
      <header className={`fixed w-full max-mobile-s:px-3 max-mobile-m:px-5 max-mobile-l:px-7 max-tablet:px-10 tablet:px-10 py-5 ${navBg ? 'bg-white transition shadow-xl ease-out' : 'bg-transparent '} z-2`}>
                    ...
                    <ul className={'text-black flex flex-col gap-5 font-semibold mb-5'}>
                        {
                            navLinks.map(link => (
                                <div  key={link.id}>
                                    <Link onClick={() => setIsSideBarOpen(false)} href={link.href}>{link.title}</Link>
